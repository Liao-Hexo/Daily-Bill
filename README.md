## 蜗牛记账-简约精致的记账app：

- 蜗牛记账是iOS平台上的一款记账产品，使用Swift语言编写

   <img src="https://raw.githubusercontent.com/Liao-Hexo/image-repository/Description/image-repository/%E8%9C%97%E7%89%9B%E8%AE%B0%E8%B4%A6/202311281751545.png" width="550px"/>

- 蜗牛记账代码使用的第三方框架

   | 名称 | 链接 |
   | :----: | :----: |
   | SnapKit | [仓库](https://github.com/SnapKit/SnapKit) |
   | SQLite.swift | [仓库](https://github.com/stephencelis/SQLite.swift) |
   | LookinServer | [官网](https://lookin.work)、[仓库](https://github.com/QMUI/LookinServer) |

- 蜗牛记账代码使用

   `git clone https://github.com/Liao-Hexo/Daily-Bill.git`    
   `pod install`
